...
function get_salesforce_fellows($fellow = null, $email = null)
{
    ...
    try {
        ...
        if ($clean_email && !$clean_fellow) {
			...
			// LFP 06/2017 added Fellowship (2) field below so that table shows the 2nd fellowship firm, if there is one.
            $query = "SELECT Id,Name,LastName,Region__c,Fellowship__c,Fellowship_2__c,AccountId,Company_Name__c,Category__c,KFP_Class__c,MailingCountry,MailingState,MailingCity,Bio_Fellowship__c,Bio_Professional__c,Bio_Education_Personal__c FROM Contact WHERE Category__c='Fellow' ORDER BY LastName";
        }
