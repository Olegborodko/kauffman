...
function get_salesforce_news($id = null, $category = null, $limit = null)
{
    ...
    try {
        ...
        if ($clean_id && !$clean_category) {
            ...
            $query = "SELECT Id,Body__c,Category__c,Title__c,StartDate__c,Sub_Title__c FROM Content__c WHERE Category__c = '$clean_category' ORDER BY StartDate__c DESC";
        } elseif ($clean_limit && $clean_category) {
            $query = "SELECT Id,Body__c,Category__c,Title__c,StartDate__c,Sub_Title__c FROM Content__c WHERE Category__c = '$clean_category' ORDER BY StartDate__c DESC LIMIT $clean_limit";
        }
